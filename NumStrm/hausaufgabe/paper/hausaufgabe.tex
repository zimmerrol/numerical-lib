...
\begin{align*}
 ...
 .
\end{align*}
Hierbei stehen $\textbf{A}, \textbf{B}, \textbf{C}$ jeweils für eine Matrix. Die Matrix $A$ 
\begin{align*}
\boldsymbol{A} =
  \begin{pmatrix}
 ...
 ,
 \end{align*}
 setzt die Randbedingungen am oberen, linken und rechten Rand ums. Die Matrix $C$
 \begin{align*}
 \boldsymbol{C}  =
  \begin{pmatrix}
  1  & 0 & . & . & 0 \\
...
dagegen setzt die Randbedingungen am unteren Rand um. Die Matrix $B$
\begin{align*}
\boldsymbol{B} =
  \begin{pmatrix}
...
Um das System nun um einen Zeitschritt weiter zu simulieren, muss die Matrix $\ŧextbf{M}$ invertiert werden - beziehungsweise das Gleichungssystem $\textbf{M} \vec{x} = \vec{b}$ gelöst werden.\\
...
Dieser Quellterm wurde in den Zeilen 148 bis 161 in (\ref{src:ftcs}) implementiert. 
...
\end{align*}
Diese Abweichung wird nun alle $100$ Zeitschritte ausgegeben. Dies wurde in der \textit{main}-Methode in Zeile 175 bis 196 implementiert.\\
...
\end{align*}
Dies liefert einen ersten Eindruck: Die Stabilitätsgrenze hängt von $Pe$ ab, und nimmt für steigende $Pe$ in gewissen, geringen Maßen in dem hier betrachteten Intervall zu.\\
...
\lstinputlisting[label=src:ftcs, title=\textbf{Quellcode:} Implementation des FTCS-Schemas]{source/task03.cpp}
\lstinputlisting[label=src:btcs, title=\textbf{Quellcode:} Implementation des BTCS-Schemas]{source/task06.cpp}

\clearpage
\subsection{Hilfsbibliotheken}
\label{sec:source_lib} 
\lstinputlisting[label=src:squarematrix.h, title=\textbf{Quellcode:} square\_matrix.h]{source/lib/square_matrix.h}
\lstinputlisting[label=src:matrix.h, title=\textbf{Quellcode:} matrix.h]{source/lib/matrix.h}
\lstinputlisting[label=src:linearsystem.h, title=\textbf{Quellcode:} linear\_system.h]{source/lib/linear_system.h}

\lstinputlisting[label=src:matrix.cpp, title=\textbf{Quellcode:} matrix.cpp]{source/lib/matrix.cpp}
\lstinputlisting[label=src:linearsystem.cpp, title=\textbf{Quellcode:} linear\_system.cpp]{source/lib/linear_system.cpp}
\end{document}